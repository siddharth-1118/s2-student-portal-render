...
export default function MarksTerminal() {
  ...
  const [editingId, setEditingId] = useState<string | number | null>(null); // Allow number IDs
  ...
  const startEdit = (entry: any) => {
    ...
    setTempMark(entry.scored); // FIX: Use .scored
  };
  ...
  const saveEdit = async (id: number) => {
    try {
      const res = await fetch('/api/marks/update', {
        ...
        // Send 'mark' as the key, but the backend maps it to 'scored'
            ...
            <tbody>
              {marksData.map((entry) => (
                <tr key={entry.id} style={{ borderBottom: '1px solid #374151', backgroundColor: editingId === entry.id ? '#374151' : 'transparent' }}>
                  ...
                  {/* DISPLAY SCORED INSTEAD OF MARK */}
